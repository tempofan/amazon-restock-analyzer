#!/usr/bin/env python3
"""
完整版云代理服务器 - 支持飞书webhook转发和领星API代理
"""
...
class CloudProxyServer:
    """
    完整版云代理服务器 - 支持WebSocket反向代理和API代理
    """
    ...
    def __init__(self, host='0.0.0.0', port=8080, ws_port=8081):
        ...
        logger.info(f"🌐 完整版云代理服务器初始化完成 - HTTP:{host}:{port}, WS:{host}:{ws_port}")
    ...
    def _register_routes(self):
        ...
        def health_check():
            ...
            return jsonify({
                ...
                'service': 'complete-proxy-server',
                'ws_port': self.ws_port,
                'active_connections': len(self.ws_clients),
                'features': ['feishu-webhook', 'api-proxy'],
                'message': '完整代理服务器运行正常',
        ...
        @self.app.route('/api/proxy/<path:api_path>', methods=['GET', 'POST', 'PUT', 'DELETE'])
        def api_proxy(api_path):
            """🔗 API代理接口 - 转发到领星API"""
            return self._handle_api_proxy(api_path)
    ...
    def _handle_feishu_request_ws(self, endpoint: str = '/feishu/webhook') -> Response:
        ...
        try:
            ...
            if response:
                ...
                flask_response.headers['X-Feishu-Proxy'] = 'Cloud-Proxy-Complete'
    ...
    def _handle_api_proxy(self, api_path: str) -> Response:
        """
        处理API代理请求 - 转发到领星API
        ...
        try:
            # 构建目标URL
            target_url = f"{self.lingxing_base_url}/{api_path}"
            ...
            logger.info(f"🔗 API代理请求: {request.method} {target_url}")
            ...
            response_time = time.time() - start_time
            logger.info(f"✅ API代理完成: {response.status_code} - {response_time:.2f}s")
            ...
            # 添加代理标识头
            flask_response.headers['X-API-Proxy'] = 'Cloud-Proxy-Complete'
            flask_response.headers['X-Response-Time'] = str(response_time)
            flask_response.headers['X-Target-URL'] = target_url

            self.stats['success_requests'] += 1
            return flask_response

        except requests.exceptions.Timeout:
            logger.warning(f"⏰ API请求超时: {api_path}")
            self.stats['failed_requests'] += 1
            return jsonify({
                'error': 'API请求超时',
                'api_path': api_path,
            ...
            error_msg = str(e)
            logger.error(f"❌ API代理异常: {error_msg}\n{traceback.format_exc()}")
            self.stats['failed_requests'] += 1
            return jsonify({
                'error': f'API代理服务器内部错误: {error_msg}',
                'api_path': api_path,
                'timestamp': datetime.now().isoformat()
            }), 500
    ...
    def run(self):
        """🚀 启动代理服务器"""
        logger.info(f"🚀 启动完整版云代理服务器: http://{self.host}:{self.port}")
        logger.info(f"📋 支持功能: 飞书webhook转发 + 领星API代理")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='完整版云代理服务器')
